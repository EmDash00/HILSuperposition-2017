...
import time
import warnings
from builtins import filter, input, object, range, str
...
from experiment.globals import (
    _GOLD,
    _PURPLE,
    ACCEL,
    ALLOW_DUPLICATES,
...
from experiment.lib.slider import sliderUSB as slider
from experiment.protocols import dynamics  # noqa: F401
...
proto = importlib.import_module("experiment.protocols." + protocol)
...
def draw_ref(scr, col, pts, thk):
    ...
    nml = old_div(perp, np.sqrt(np.sum(perp**2, axis=1))[:, np.newaxis])
    pts_u = pts[:-1] + 0.2 * thk * nml
    pts_d = pts[:-1] - 0.2 * thk * nml
...
class Point(object):
    ...
    def __div__(self, scalar):
        return Point((old_div(self.x, scalar), old_div(self.y, scalar)))
...
def draw_dashed_line(surf, color, start_pos, end_pos, width=1, dash_length=10):
    ...
    slope = old_div(displacement, length)

    for index in range(0, old_div(length, dash_length), 2):
...
def rescale_inp(inp, MIN=SLIDER_MIN, MAX=SLIDER_MAX):
    out = 2 * (old_div((inp - MIN), (MAX - MIN)) - 0.5) * trial["scale"] * (3.0 / 2.0)
    # print("Input:", out)
    return out
    ...
    if not ALLOW_DUPLICATES:
        while (
            len(
                glob.glob(
                    os.path.join(
                        subject_dir, "*" + protocol + "_" + str(trial["id"]) + ".npz"
                    )
                )
            )
            > 0
...
trial_reset = dict(
    ...
    scale=0.5,
    init=[0.0],
    dis=lambda t, x, _: 0.0,
    out=lambda x: x[0],
    ref=lambda t, _: 0.0 * np.asarray(t),
    vf="fo",
)
...
def init(trial):
    ...
    ref_ = [trial["ref"](time_[-1], trial) * (RANGE[1, 1] - RANGE[0, 1])]
...
def rk_(vf, t, x, u, d=None, dt=1.0):
    dx1 = vf(t, x, u, d) * dt
    dx2 = vf(t + 0.5 * dt, x + 0.5 * dx1, u, d) * dt
    dx3 = vf(t + 0.5 * dt, x + 0.5 * dx2, u, d) * dt
    dx4 = vf(t + dt, x + dx3, u, d) * dt
    dx = (1.0 / 6.0) * (dx1 + 2 * dx2 + 2 * dx3 + dx4)
    return x + dx
...
if PLOT:
    ...
    fig = plt.figure(1, figsize=(old_div(size[0], 100), old_div(size[1], 100)))
    plt.clf()
    plt.grid(True)
    plt.axis("equal")
    plt.xlim(RANGE[:, 0])
...
while not done:
    ...
    if not PAUSE:
        ...
        if TRIAL_STATE == "reset1":
            if np.abs(out_[-FPS * SLIDER_SPT :]).max() > 0.5:
                save(
                    time_=time_,
                    realtime_=realtime_,
                    state_=state_,
        ...
        if TRIAL_STATE in ["run", "reset1", "reset2"]:
            ...
            if TRIAL_STATE in ["run"]:
                state = rk_(
                    eval("dynamics." + trial["vf"]), _time, state, _inp, _dis, STEP
                )
            else:
                state = [_inp]
            # --- record game state
            ...
            ref_.append(
                trial["ref"]([SHIP_SHIFT + _time, _time], trial)[0]
                * (RANGE[1, 1] - RANGE[0, 1])
            )
    ...
    if steps % SLIDER_SPT == 0 and not done:
        if TRIAL_STATE in ["run"]:
            ...
            print("UNKNOWN TRIAL STATE, USING DEFAULT COLORS.")
            bg_color = BLACK
            ref_color = GOLD
            out_color = PURPLE
        ...
        if SHOW_GRID:
            xticks = np.arange(old_div(-RATIO, 2), old_div(RATIO, 2), 0.5)
            for g in xticks:
                draw_dashed_line(
                    screen,
                    DARKGREY,
        ...
        if TRIAL_STATE in ["run"]:
            ...
            TIMES = np.linspace(
                -0.5 - THICK_REFERENCE, 0.5 + THICK_REFERENCE, old_div(size[0], 10)
            )
            ...
            # reference curve
            if SHOW_REF:
                rects.append(draw_ref(screen, ref_color, pts, THICK_REFERENCE))
            # reference glyph
            rects.append(
                draw_rect(
                    screen,
            ...
            rects.append(
                draw_rect(
                    screen,
                    ref_color,
                    (old_div(-RATIO, 2), 0.5, old_div(RATIO, 2) - 0.5, 1.0),
                    THICKNESS_SYS,
                )
            )
            rects.append(
                draw_rect(
                    screen,
                    ref_color,
                    (0.5, 1.0, old_div(RATIO, 2) - 0.5, old_div(+RATIO, 2)),
                    THICKNESS_SYS,
                )
            )
        ...
        # Create as lists and convert to array
        # pts = np.asarray(
        # [[out_[-1], 0], [-RAD_SYS, 0], [0, RAD_SYS], [RAD_SYS, 0], [0, -RAD_SYS]]
        # )
        rects.append(draw_polygon(screen, out_color, pts, THICKNESS_SYS))
        if SHOW_INPUT:
            # input
            rects.append(
                draw_lines(
                    ...
                    [
                        [0.0, INPUT_SHIFT],
                        [old_div(_inp, (trial["scale"])), INPUT_SHIFT],
                    ],
                    THICKNESS_INPUT,
                )
            )
        if SHOW_DISTURBANCE:
            ...
            rects.append(
                draw_lines(
                    ...
                    False,
                    [
                        [0.0, DISTURBANCE_SHIFT],
                        [old_div(_dis, (trial["scale"])), DISTURBANCE_SHIFT],
                    ],
                    THICKNESS_DISTURBANCE,
                )
            )
        ...
        activerects = list(filter(bool, activerects))
        ...
        if PLOT:
            ...
            plt.plot(
                TIMES,
                trial["ref"](TIMES + _time, trial) * (RANGE[1, 1] - RANGE[0, 1]),
                ".-",
                lw=10,
                color=_GOLD,
            )
        ...
        if FADE < 0:
            fader.fill((0.0, 0.0, 0.0, 255 + FADE))
            screen.blit(fader, (0, 0))
            FADE -= 20
            if FADE < -255:
                FADE = 0
    ...
    if (steps + 1) % 60 == 0:
        dbg(
            "%d t = %0.1f, FPS = %0.1f, inp = %0.1f, state = %s"
