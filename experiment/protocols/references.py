...
def spline_interp(x, x0, x1, y0, y1, dy0, dy1, return_dy=False, return_ddy=False):
    ...
    if np.allclose(x1, x0) or np.isinf(x0) or np.isinf(x1):
        return y0
    t = old_div((x - x0), (x1 - x0))
    a = dy0 * (x1 - x0) - (y1 - y0)
    b = -dy1 * (x1 - x0) + (y1 - y0)
    y = (1 - t) * y0 + t * y1 + t * (1 - t) * (a * (1 - t) + b * t)
    dy = (
        old_div((y1 - y0), (x1 - x0))
        + old_div((1 - 2 * t) * (a * (1 - t) + b * t), (x1 - x0))
        + old_div(t * (1 - t) * (b - a), (x1 - x0))
    )
    ddy = old_div(2 * (b - 2 * a + (a - b) * 3 * t), (x1 - x0) ** 2)
    r = [y]
    if return_dy:
        r.append(dy)
    if return_ddy:
...
def spline(t, trial, x=None):
    ...
    t = np.asarray(t)
    conds = np.asarray(
        [np.logical_and(t0 <= t, t <= t1) for t0, t1, y0, y1 in trial["pts"]]
    )
    if t.size == 1:
    ...
    if len(keep) > 0:
        conds = conds[keep]
        funcs = [
            lambda t, t0=t0, t1=t1, y0=y0, y1=y1: spline_interp(
                t, t0, t1, y0, y1, 0.0, 0.0
            )
...
def sum_of_sines_ramp(
    t, trial, x=None, frequencies=None, amplitudes=None, phase_shifts=None
):
    t = np.asarray(t).copy()
    t.shape = (t.size, 1)
    ...
    if r > 0:
        _ *= ((old_div(t * (t <= r), r) + (t > r)).flatten()) ** 2
...
def sum_of_sines(
    ...
    assert f.shape == a.shape == p.shape, (
        "Shape of frequencies, amplitudes, and phase shifts must match"
    )
